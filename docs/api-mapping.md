...
| `StringDecoder` | no public v0 equivalent | deferred | Batch decode uses local UTF/table codecs; stream-safe incremental decoding is deferred. |
...
| `defaultCharUnicode`, `defaultCharSingleByte` | fixed substitution behavior | adapted | C++ v0 uses U+FFFD on decode and upstream `?` substitution rules on encode. |
